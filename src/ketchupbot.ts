...
import * as config from './ketchupconfig.json'
...
export class ShipUpdater {
	...
	async updateGalaxypediaShips(onlyship?: string) {
		...
		try {
			if (this.currentlyUpdating) {
				...
				return "Already updating ships; not updating"
			}

			process.env.SHIP = onlyship || process.env.SHIP
			...
			process.env.SHIP = "" || process.env.SHIP
		} catch (error: any) {
